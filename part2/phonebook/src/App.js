...
const App = () => {
  ...
  const addPerson = (event) => {
    ...
    const differentName = persons.every((person) =>
      notEqual(person, { name: newName })
    );
    if (differentName) {
      const personObj = {
        name: newName,
  ...
  const updatePerson = () => {
    const person = persons.find(
      (person) => newName.toLocaleLowerCase() === person.name.toLowerCase()
    );
    const id = person.id;
  ...
  return (
    <div>
      ...
      <PersonForm
        onSubmit={addPerson}
        nameValue={newName}
        nameChnage={handleNameChange}
        numberValue={newNumber}
        numberChnage={handleNumberChange}
      />
      <h3>Numbers</h3>
      <Person
        persons={persons}
